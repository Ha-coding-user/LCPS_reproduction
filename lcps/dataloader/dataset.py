...
class spherical_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if type(labels)==np.ndarray and type(insts)==np.ndarray:
            ...
            voxel_label = nb_process_label(np.copy(voxel_label), label_voxel_pair.astype(np.int64))
            ...
            if voxel_inst.size > 1:
                ...
                processed_inst = nb_process_inst(np.copy(processed_inst), inst_voxel_pair.astype(np.int64))
            else:
