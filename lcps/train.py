...
def main():
    ...
    # Debugging --> 필요 없을 시 삭제
    #parser.add_argument('--debug', actioni='store_true', help='debugging_mode')
    ...
    if args.resume:
        ...
        # Eliminate extra modules when saving parameters during distributed training.
        weights_dict = {}
        for k, v in pretrained_model.items():
            new_k = k.replace('module.', '') if 'module' in k else k
            weights_dict[new_k] = v
        # # Check the parameters when debugging
    ...
    # DDP's sync_bn makes the bn range of Doka training normal
    if args.local_rank != -1:
